...
微端包在热更新完成后有可能会碰到游戏so访问资源失败的问题，游戏so中如果访问了apk里的资源，则需要对游戏apk中的so进行处理。
...
<settings>
    ...
    <!--    需要做处理的补丁包里的so的列表，用逗号隔开。例如"libtest1.so,libtest2.so"。 仅在hot_update为true的情况下才有意义。 -->
    <setting key="update_so_list" value="libunity.so,libxxx1.so,libxxx2.so" />
</settings>
```

将debug的值改为true；将游戏apk中lib下的所有so文件填写到update_so_list字段的value字段中并用英文逗号隔开。

第三步，编译生成微端包并运行到手机，执行热更新流程。

第四步，查找哪些so访问了游戏资源。热更新完成点击立即重启，同时log中过滤“redirect so”字段，查看过滤的结果中包含了哪些so，

将这些so保留，其他so从update_so_list中移除，并把debug字段改为false，重新生成微端包并验证热更新流程。
...
如果您的游戏apk使用了加固方案，那就需要在配置文件中设置对应参数，不同的加固厂商对应的flag不同，

目前兼容了腾讯ACE加固和网易易顿加固，

腾讯ACE加固默认不需要更改，网易易顿需要修改update_flag字段，

打开工程assets下的microSettings.xml，修改以下参数，

```xml
<?xml version="1.0" encoding="utf-8"?>
<settings>
    <!--    兼容易顿加固，请设置该flag。仅在hot_update为true的情况下才有意义。 -->
    <setting key="update_flag" value="0,1,0,0,0,1" />
</settings>
```
