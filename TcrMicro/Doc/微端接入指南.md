...
游戏apk在被加固后可能有兼容问题，目前经过验证的第三方加固有腾讯ACE加固。

游戏apk使用腾讯ACE加固之后，需要修改assets下的microSettings.xml文件，

```xml
<?xml version="1.0" encoding="utf-8"?>
<settings>
    ...
    <!--    是否使用了ace加固，目前微端支持ace加固。 仅在hot_update为true的情况下才有意义。 -->
    <setting key="use_ace_encryption" value="true" />
</settings>
```
