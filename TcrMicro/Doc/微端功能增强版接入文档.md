...
<img
src="https://cg-sdk-1258344699.cos.ap-nanjing.myqcloud.com/micro/docs/picture/%E5%8A%9F%E8%83%BD%E5%A2%9E%E5%BC%BA%E7%89%88.png" width="600px">

功能增强版在快速接入的基础上新增了静默安装、登录/支付穿透的功能，主要的流程如下：

在微端示例工程的基础上修改配置，接入游戏登录/支付SDK，生成微端包。

在游戏原始包中接入云端SDK，将登录/支付逻辑迁移到微端包中，生成云端包部署到腾讯云服务器。

使用patch工具将游戏原始包转化为补丁包，微端包运行过程中下载补丁包，静默更新到完整包。

## 2. 功能接入指南

### 2.1 登录/支付穿透

不需要登录/支付穿透的直接把游戏原始包给到工作人员部署到腾讯云服务器，生成对应的gameId即可。

<img
src="https://docimg7.docs.qq.com/image/7QWjLfYrV-ERh6pR0lFN6g.png?w=1280&h=657.7628259041211" width="600px">

登录/支付穿透的主要流程是，云端包在腾讯云服务器中7*24小时运行并开启数据通道监听，每个微端包在启动之后会向云服务器请求一个云游戏实例，进入云游戏之后连接数据通道，用户点击登录，云端包通过数据通道告知微端包执行登录逻辑，微端包将处理之后的登录结果再通过数据通道发送给云端包，云端包收到之后执行登录之后的逻辑。支付操作同理。
...
云端sdk的具体使用方法请参考[云端SDK的API文档](云端SDK的API文档.zip)，同时云端包的配置可以参考[云端APP示例工程](../Demo/云端APP示例工程/TcrMicroCloud.zip).
...
```java
        testButton.setOnClickListener(v -> {
            new Thread(new Runnable() {
                ...
                public void run() {
                    try {
                        String testData = "testData"; // 修改测试数据
...
在微端示例工程的基础上接入登录/支付SDK，实现登录/支付逻辑，将处理的结果发送给云端包。
...
第一步，安装java11环境，在AndroidStudi中打开[微端APP示例工程](../Demo/微端APP示例工程/TcrMicroAppForUnity2018Empty.zip)，在Terminal中进入到hotUpdate目录下，使用以下命令生成补丁包：

```shell
./generate_patch.sh 游戏原始包路径
```

该脚本通过游戏原始apk生成补丁包。补丁包生成路径为：hotUpdate/out/patch.apk。

微端包经过热更新配置之后加载该补丁更新到完整包。

#### 2.2.2 微端包静默更新配置

为了保证微端包能够正常升级到完整包，需要依赖游戏原始包的R.txt文件和AndroidManifest.xml清单文件。R.txt文件保证热更新前后的resource ID保持不变，清单文件保证热更新前后app组件保持不变。

**准备工作**：

第一步，通过游戏原始包获取R.txt文件。
...
第二步，通过游戏原始包获取AndroidManifest.xml文件。
