...
下载[数据通道测试APP](../Demo/云端APP示例工程/DataChannelTest.zip)，解压之后通过AndroidStudio打开。
...
该脚本主要通过aapt2命令提取游戏原始apk中的资源名称和ID，生成微端包需要的R文件（保证热更新之后相同的资源ID不变）。

注意：使用该脚本之前需要将脚本中的ANDROID_HOME替换为自己本地的路径，选择自己本地的build-tools的版本。
...
注意：数据发送必须要在onDataCHannelConnectSuccess之后进行，收到该回调后还需要向云端APP发送一条确认消息来告诉云端APP微端APP已经连接成功（参考sendConnectMsg方法）。

第七步，构建生成微端apk，并校验微端apk生成是否正常。
...
该脚本通过检查微端apk的修改是否能够正常更新到游戏原始apk，并输出检查结果。
