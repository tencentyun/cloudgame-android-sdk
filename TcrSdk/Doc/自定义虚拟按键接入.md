...
1、集成SDK。在应用模块的`build.gradle`中加入：

```groovy
dependencies {
	 ......
    implementation 'com.tencent.tcr:tcrsdk-full:2.0.0'
    implementation 'com.tencent.tcr:tcrui:2.0.0'
}
```

2、AndroidManifest 配置网络权限，虚拟按键和云端的交互需要网络的请求：
...
3、初始化[云游SDK](入门指南.md)，虚拟按键的按键需要在初始化云游SDK后才能使用。

4、初始化虚拟按键视图，加载从文件读取的布局并展示

``` java
mGamePadManager = new GamepadManager(getApplicationContext());
mCustomGamePadCfg = readConfigFile("lol_5v5.cfg");  //读取文件方式可自行实现
mGamePadManager.showGamepad(mCustomGamePadCfg);  // 加载按键布局
```
...
```
7、设置触摸事件代理(您也可以使用TcrSdk中的设置PcTouchHandler、MobileTouchHandler来处理触摸事件)

```
mGamePadManager.setGamePadTouchDelegate(new IGamepadTouchDelegate() {
            @Override
            public boolean onTouch(View view, MotionEvent motionEvent) {
                mRenderView.handleMotion(motionEvent);
                return true;
            }
        });
...
8、您可以在需要的地方打开或关闭虚拟键盘和自定义编辑按键

```java
editCustomGamePad(true); // 打开自定义编辑按键
keyboard(true); // 打开虚拟键盘
```
