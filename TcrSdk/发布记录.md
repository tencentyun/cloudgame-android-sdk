...
**Features**
- `TcrSession.Event#STATE_CLOSED` 新增连接失败细分错误码：`CONNECT_FAILED_SERVER_REJECTED`(104009)、`CONNECT_FAILED_SDP`(104010)、`CONNECT_FAILED_DISCONNECTED`(104011)。（2026-08-13）
- `TcrSession.Event` 新增 `FIRST_FRAME_RECEIVED`（收到云端首帧视频）事件。（2026-01-12）
- `TcrSession.Event` 新增 `TOKEN_EXPIRED`（鉴权 token 过期）事件。（2026-06-22）
- `TcrSessionConfig` 新增 `sessionMode` 字段，支持 `"ExclusiveSession"`（断开其他客户端的串流连接）与 `"SharedSession"`（保留其他客户端连接，默认）。（2026-01-21）
- `PcTouchListener` 新增 `lockMouse(boolean)` 接口，用于锁定/解锁本地鼠标光标（锁定时云端隐藏光标）。（2026-01-23）
- `AccessInfo` 新增 `WebRTCBackupAddress` 字段，`TcrSession#play()` 时会自动使用备用地址重试连接。（2026-01-14）
- `StatsInfo` 新增 `raw_rtt`、`rtt`、`edge_rtt` 字段，其中 `edge_rtt` 为边缘服务器测得的网络往返时延。（2025-12-10 / 2025-12-11）
- 移除 `TcrSession#setRemoteVideoProfile()` 接口的废弃标记，恢复为可用状态。（2025-12-01）
- 检测到设备为模拟器时自动改用软解，解决 MuMu 模拟器等模拟器解码慢的问题。（2026-01-28）

**Bug Fixes**
- 修复 `TcrRenderViewType.TEXTURE` 渲染模式下偶现崩溃的问题。（2025-12-12）
- 修复 `FIRST_FRAME_RECEIVED` 事件回调多次的问题。（2026-01-12）
- 修复未设置 `SdkType` 时断网自动重连失败的问题。（2026-03-06）
- 修复骁龙 8 Elite Gen 5 等机型 H265 硬解兼容性问题，异常时自动回退软解。（2026-07-27）
- 修复 `SurfaceTextureHelper` 偶现崩溃问题（更新 twebrtc 库）。（2026-03-19）

**特别注意（行为变更）**
- 连接失败时，SDK 不再回调 `TcrCode.SessionStop#CONNECT_FAILED`(104007)，改为回调上述细分错误码。若您的代码对 104007 有精确匹配判断，请适配为细分错误码，或将 104007 作为未知连接失败的兜底处理。

# [Version 3.34.0](https://github.com/tencentyun/cloudgame-android-sdk/tree/tcrsdk/3.34.0/TcrSdk) (2025-11-6)
**Features**
- 云手机PaaS支持网络重定向功能。
- 不上行静音帧。
- 新增接口 TcrSession#inputText() 用于本地输入法。
- 修改接口 CustomVideoI420Helper#buildFrame() 增加回调参数。

**Bug Fixes** 
- 修复了 TcrSession#access() 回调问题。
- 修复了 TcrRenderView 帧回调花屏问题。
- 修复了 ScreenConfig 的问题。
- 修复了一些其他问题。


# [Version 3.33.4](https://github.com/tencentyun/cloudgame-android-sdk/tree/tcrsdk/3.32.0/TcrSdk) (2025-11-04)
**Bug Fixes** 
- 解决视频流分辨率变化时`TcrRenderView.Observer#onFrame`回调的Bitmap出现花屏问题

# [Version 3.33.1](https://github.com/tencentyun/cloudgame-android-sdk/tree/tcrsdk/3.32.0/TcrSdk) (2025-10-29)
**Bug Fixes** 
- 修复云手机PasS无法使用网络重定向功能问题

# [Version 3.33.0](https://github.com/tencentyun/cloudgame-android-sdk/tree/tcrsdk/3.33.0/TcrSdk) (2025-10-20)
**Features**
-  云手机 SFU 相关功能：
    1. 设置视频宽高接口 TcrSession#setRemoteVideoProfile()。
    2. 暂停恢复推流时可以指定音视频轨。TcrSession#pauseStreaming(type)和TcrSession#resumeStreaming(type)接口。
    3. 创建自定义数据通道 TcrSession#createCustomDataChannel() 接口增加 type 参数。
    4. TcrSessionConfig 增加 streamProfile 字段配置初始流，增加客户自定义 userId。
    5. SDK 上行的触摸事件新增 pressure 等字段信息。
    6. 中途加入群控 joinGroupControl 新实现（通过 DataChannel）。
    7. 云手机串流场景，支持自动断网重连。
- 实现自定义视频上行功能，TcrSession#sendCustomVideo()接口， TcrSessionConfig#enableCustomVideoCapture 配置， CustomVideoI420Helper工具类， CustomVideoTextureHelper工具类。 
- libtwebrtc.so 支持 16KB 页面。

**Bug Fixes** 
- 修复App屏幕旋转方向错误的问题。
- 修复一些其他问题。

# [Version 3.32.0](https://github.com/tencentyun/cloudgame-android-sdk/tree/tcrsdk/3.32.0/TcrSdk) (2025-9-4)
**Features**
-  新增能力：PcTouchListener 内部自动处理 CURSOR_STATE_CHANGE 事件。
