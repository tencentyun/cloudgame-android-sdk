...
在游戏原始包中接入热更新SDK，编译生成更新包，将生成的更新包保存。详细流程请参考【生成更新包】。
...
将保存的微端包和更新包通过命令行工具生成补丁包，请参考【生成补丁包】，将生成的补丁包通过微信对接群给
...
更新包不被安装运行，它的作用是和微端包对比生成补丁包。生成更新包的工程是由原始游戏工程集成热更

新框架而成。下面详细介绍构建更新包工程和APK的步骤：
...
微端APK是最终投放给用户的，用户下载安装运行到手机上，提供云试玩和热更新到更新游戏的能力。您可以下载
...
将更新包的AndroidManifest.xml文件拷贝到微端工程的对应目录下，替换同名文件。在该文件里声明的
...
将1.3节更新包工程里生成的构建物（xxx.apk和xxx-R.txt）拷贝到hotUpdate文件夹下。
...
将前面生成的微端apk拷贝到tools目录下，重命名为old.apk。将前面生成的更新包apk拷贝到tools目录下，重命名
...
[更新包示例工程](https://recorder-10018504.cos.ap-shanghai.myqcloud.com/shaw/微端接入资源汇总/热更APP示例工程/AndroidUnity2018Empty.zip)
...
首次接入时，微端包采用的sdk版本和更新包采用的版本是一致的，不需要进行该操作。

实际调试中，如果修改了微端包 sdk的版本，但是更新包还是使用之前的版本（尽量减少更新包的修改）。
...
第三步，筛选出更新包apk比微端包apk多出来的so，libunity.so、libil2cpp.so、libmain.so，将这三个so添加到
